...
libbuild.REPO_ROOT = expandvars('$GOPATH') + '/src/github.com/appscode/go-seed-cli'
...
def update_registry():
    vf = libbuild.REPO_ROOT + '/dist/go-seed-cli/versions.json'
    bucket = libbuild.BUCKET_MATRIX.get(libbuild.ENV, libbuild.BUCKET_MATRIX['dev'])
    call('gsutil cp {0}/binaries/go-seed-cli/versions.json {1}'.format(bucket, vf))
    vj = {}
    ...
    call("gsutil cp {1} {0}/binaries/go-seed-cli/versions.json".format(bucket, vf))
    call('gsutil acl ch -u AllUsers:R -r {0}/binaries/go-seed-cli/versions.json'.format(bucket))

    lf = libbuild.REPO_ROOT + '/dist/go-seed-cli/latest.txt'
    libbuild.write_file(lf, BUILD_METADATA['version'])
    call("gsutil cp {1} {0}/binaries/go-seed-cli/latest.txt".format(bucket, lf))
    call('gsutil acl ch -u AllUsers:R -r {0}/binaries/go-seed-cli/latest.txt'.format(bucket))
